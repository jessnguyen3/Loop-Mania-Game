...
/**
 * objects of this class represents the position in the path. it holds the
 * current position in the path and a reference to the orderedPath internally.
 * It also holds SimpleIntegerProperties for x and y coordinates, which we have
 * getter methods for, so we can return them and attach ChangeListers, to
 * decouple the frontend and backend. The SimpleIntegerProperties are updated
 * automatically when we move through the path.
 */
public class PathPosition {
    ...
    /*
     * the integerproperty internal int is the x or y coordinate - since want to use
     * this class to bind with node positions currentPositionInPath is an index of
     * orderedPath usingValue0 is true if using value0 in path, false if using
     * value1 orderedPath is the list of path coordinates in pairs
     */
